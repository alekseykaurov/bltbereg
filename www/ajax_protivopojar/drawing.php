...
if($main_lock!=""){
	...
	$main_lock_isNajim = $modx->getTemplateVars(Array("najim"), '*', $main_lock);
	...
	$result["main_lock_najim"] = $main_lock_isNajim[0]["value"];
} else {
	$result["main_lock"] = "";
...
//----RUCHKA----------
if($ruchka!="" && $ruchka!=undefined){
	$ruchka_picture = $modx->getTemplateVars(Array("option_image"), '*', $ruchka);
	if($ruchka_picture[0]["value"]!=""){
		$result["ruchka"] = $ruchka_picture[0]["value"];
	} else {
		$result["ruchka"] = "";
	}
	$antipanika_tv = $modx->getTemplateVars(Array("antipanika"), '*', $ruchka);
	$result['antipanika'] = $antipanika_tv[0]['value'];
} else {
	$result["ruchka"] = "";
}
