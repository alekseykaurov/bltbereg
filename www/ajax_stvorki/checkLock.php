...
$default_main_lock = isset($_POST['default_main_lock']) ? $_POST['default_main_lock'] : "";
...
// 					$first_child_ispp = $modx->getTemplateVars(Array("is_pp"), '*', $childs[0]["id"]);
// 					if($first_child_cilindr[0]["value"]=="DORMA" || $first_child_color[0]["value"]!=$add_lock_color[0]["value"] || $first_child_ispp[0]["value"]=="true"){
// 						//прогоняем все замки и ищем по такому же фильтру, как у текущего замка
// 						foreach($childs_sorted as $key => $value){
// 							if($new_main_lock == ""){
// 								$current_cilindr = $modx->getTemplateVars(Array("cilindr"), '*', $value["id"]);
