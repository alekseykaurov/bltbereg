		...
		if($value[2]=="yes"){ //если это мангал
			...
			if ($row = mysql_fetch_assoc($res)) {
			 ...
			 //    	$door_name = "<a href='http://ce77747.tmweb.ru/konstruktor-protivopozharnyix-dverej/?project=".$value[0]."'>Мет.дверь, 1-ств, противопожарная пр.№".$value[0]."</a>";
				...
				// 	$door_name = "<a href='http://ce77747.tmweb.ru/slozhnyie-dveri-(testyi).html?project=".$value[0]."'>Мет.дверь, 2-ств, противопожарная пр.№".$value[0]."</a>";
				// } else {
				...
				if($row["door_type"]== 1 || $row["door_type"]=="protivopojar"){
			    	$door_name = "<a href='http://blt-bereg.ru/konstruktor-protivopozharnyix-dverej/?project=".$value[0]."'>Мет.дверь, 1-ств, противопожарная пр.№".$value[0]."</a>";
				} else if ($row["door_type"] == 2 || $row["door_type"] == 3) {
					$door_name = "<a href='http://blt-bereg.ru/konstruktor-protivopozharnyix-dverej/?project=".$value[0]."'>Мет.дверь, 1-створ. с глух. частью, противопожарная пр.№".$value[0]."</a>";
				} else if($row["door_type"] == 4){
					$door_name = "<a href='http://blt-bereg.ru/konstruktor-protivopozharnyix-dverej/?project=".$value[0]."'>Мет.дверь, 2-створ., противопожарная пр.№".$value[0]."</a>";
				} else if($row["door_type"] == 5){
					$door_name = "<a href='http://blt-bereg.ru/konstruktor-protivopozharnyix-dverej/?project=".$value[0]."'>Мет.дверь, 1-створ. с 2-мя глух. частями, противопожарная пр.№".$value[0]."</a>";
				} else if($row["door_type"] == 6){
					$door_name = "<a href='http://blt-bereg.ru/konstruktor-protivopozharnyix-dverej/?project=".$value[0]."'>Мет.дверь, 2-створ. с глух. частm., противопожарная пр.№".$value[0]."</a>";
					...
					$door_name = "<a href='http://blt-bereg.ru/konstruktor-dverej/?project=".$value[0]."'>Мет.дверь, 1-ств, квартирная пр.№".$value[0]."</a>";
				}
			} else {
...
// $to = "ekaterina.bidyanova@yandex.ru";
$to = "bmihh@yandex.ru, blt-bereg@mail.ru";
$from = "admin@blt-bereg.ru";
$boundary = md5(date('r', time()));
$headers .= "From: Blt-Bereg.ru <" . $from . ">\r\n";
