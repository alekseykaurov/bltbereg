...
class CJot {
	...
	function Run() {
		...
		$this->config["subject"]["author"] = !is_null($this->Get("subjectAuthor")) ? $this->Get("subjectAuthor") : "Новый отзыв на сайте";
	...
	function getOutputComments() {
		...
		if($this->canView) {
				...
				for ($i = 0; $i < $count; $i++) {
					...
					$new_start= substr($array_comments[$i]["content"], 0, 300);
					$new_end= substr($array_comments[$i]["content"],300);
					$array_comments[$i]["content"]=$new_start;
					$array_comments[$i]["content_end"]	= $new_end;		
				}
				...
				for ($i = 0; $i < $count; $i++) {
					...
					$tpl->AddVar('cid',$i+1); // id для комментария
					$comments[] = $tpl->Render();
				}
	...
	function doNotifyModerators($commentid=0) {
		...
		if ($this->config["moderation"]["notify"]) {
		  ...
		  // Get comment fields
			...
			/* получатели */
$to= "marketing@blt-bereg.ru, 9882313@blt-bereg.ru " ; //обратите внимание на запятую
...
<body>
<p>На сайте добавлен новый отзыв!</p>
<p>Просмотерть отзыв можно на странице: <a href="http://blt-bereg.ru/otzyivyi.html">otzyivyi.html</a></p>
</body>
</html>
';
...
$headers .= "From: blt-bereg.ru <info@blt-bereg.ru>\r\n";
